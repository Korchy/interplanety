		...
		if(this._galleries[res[1]])g=this._galleries[res[1]];
			...
			g={'name':res[1],'a':[],
				...
				'setNav':function(){
					...
					if(this.current>0){
						var left=e.appendChild(todo.create('div',{'class':'todo_gallery_button_left','title':'Назад'}));
						left._gallery=this;
						left.onclick=function(){this._gallery.prev();};
					};
					if(this.current<this.a.length-1){
						var right=e.appendChild(todo.create('div',{'class':'todo_gallery_button_right','title':'Далее'}));
						right._gallery=this;
